...
def update_comment(request: HttpResponse, slug_category, slug_article, pk: int):
    ...
    if request.method == "POST":
        comment_data = {"content": request.POST.get("content")}
        comment_form = CommentForm(data=comment_data)
        if comment_form.is_valid():
...
def get_story_and_comments(request: HttpResponse, slug_category, slug_article):
    ...
    comment_form = CommentForm()
    likes_article = (
        LikeArticle.objects.values("article_id")
        .filter(article_id=art)
        .annotate(number=Count("article_id"))
    )

    dislikes_article = (
        DislikeArticle.objects.values("article_id")
        .filter(article_id=art)
        .annotate(number=Count("article_id"))
    )
...
def get_favorite_article(request):
    ...
    return render(
        request,
        "my_page.html",
        context={"favorite_art": favorite_art, "user": user_, "words": words},
    )
...
def _translate(word, language):
    ...
    Translation.objects.create(
        word=word, translation=translated_word, language=language
    )
...
def get_test(request: HttpResponse):
    ...
    return render(
        request,
        "result.html",
        context={"wrong": wrong, "correct": correct, "total": total, "x": x},
    )
...
def get_test_language(request: HttpResponse):
    ...
    if request.method == "POST" and form_language.is_valid():
        ...
        return render(
            request, "test.html", context={"language": lang_object, "words": words}
        )

    return render(request, "test_language.html", context={"form": form_language})
